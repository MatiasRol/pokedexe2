  ...
  const toggleIA = () => {
    if (iaVisible) {
      Animated.timing(slideAnim, {
        toValue: SCREEN_WIDTH, // fuera de pantalla
        duration: 400,
      ...
      <ScrollView className="flex-1">
        <View className="p-4 max-w-2xl mx-auto w-full">
              ...
              {!loading && !error && pokemon && (
                <View>
                  ...
                  <View className="items-center mb-4 bg-white rounded-2xl p-4 shadow-md border-2 border-gray-300">
                    ...
                    {/* Botón pregunta IA (restaurado abajo de la info) */}
                    <TouchableOpacity
                      onPress={() => {
                        toggleIA();
                        setMessages([{ 
                  ...
                  <View className="flex-row gap-2 mb-6">
                    <TouchableOpacity
                      ...
                      className={`flex-1 py-4 rounded-2xl items-center border-2 shadow-lg ${pokemon.id <= 1 ? 'bg-gray-300 border-gray-400' : 'bg-blue-500 border-blue-700'}`}
                    >
                      <Text className={`font-bold text-base ${pokemon.id <= 1 ? 'text-gray-500' : 'text-white'}`}>← Anterior</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                      ...
                      className={`flex-1 py-4 rounded-2xl items-center border-2 shadow-lg ${pokemon.id >= 1000 ? 'bg-gray-300 border-gray-400' : 'bg-blue-500 border-blue-700'}`}
                    >
                      <Text className={`font-bold text-base ${pokemon.id >= 1000 ? 'text-gray-500' : 'text-white'}`}>Siguiente →</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              )}
          ...
          <View className="items-center mt-6 pb-8">
            <Text className="text-white text-sm font-bold drop-shadow-lg">🎮 Pokédex Clásica con IA</Text>
        ...
        >
          ...
          <ScrollView className="flex-1 mb-3 bg-white rounded-2xl p-3 border-2 border-gray-300">
            ...
            {messages.map((msg, index) => (
              <View
                key={index}
                className={`my-2 px-4 py-3 rounded-2xl max-w-[85%] shadow-md ${msg.role === 'user'
                    ? 'bg-purple-600 self-end border-2 border-purple-800'
                    : 'bg-gray-100 self-start border-2 border-gray-300'
                  }`}
              >
                <Text className={`${msg.role === 'user' ? 'text-white' : 'text-gray-900'} font-medium`}>
                  {msg.text}
                </Text>
              </View>
            ))}
          ...
          <View className="flex-row gap-2">
            ...
            <TouchableOpacity
              ...
              className={`px-5 py-3 rounded-2xl border-2 shadow-lg ${!currentMessage.trim() || iaLoading
